...
```{r}
#another way to calculate mean and sd
...
ggplot(mapping=aes(x=Type_of_PFAS, y=Conc_of_PFAS, fill=Type_of_PFAS))+
  geom_boxplot(outlier.color = "darkred", outlier.shape=16, outlier.size = .6, notch=FALSE,aes(y=log(Conc_of_PFAS)))+ #log of conc. values
  geom_line(data=data_pfas_sd, aes(x = Type_of_PFAS, y = log(Conc_of_PFAS)), size = 1, color="red", group = 1)+ #log of mean values
